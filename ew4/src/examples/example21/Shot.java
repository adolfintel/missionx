...
public class Shot extends SpriteColl implements ILoopable {
    ...
    public Shot(float x, float y) {
        ...
        direction = new Vector2f(Mouse.getX() - getX(), Mouse.getY() - getY());
        direction.normalise();
        direction.scale(speed);
        ManagedGroupCollision.get("Shots").addToDestination(this);
        setRGBA(1, 0, 0, 1);
    }
    ...
    private boolean checkBounds() {
        ...
        // out of screen test!
        if (!CollisionUtils.rectangleHit(this, Camera.getMainFather())) {
            destroy();
            return true;
        }
        return false;
