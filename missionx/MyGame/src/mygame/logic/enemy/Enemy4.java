 ...
 * @author Do$$e
 */
public class Enemy4 extends SpriteColl implements ILoopable {
    ...
    float scale = 1f;
    Emitter explosion, shockWave, engine1;
    Texture ship, ship_hit;
    int cycle = 0, initHealth, randY = (int) (Math.random() * 100), kamikazeRandom = (int) (Math.random() * 10) - 5;
    float health = 35;
    ...
    public Enemy4(float x, float y, int health) {
    ...
    public void init() {
        ...
        ship = Texture.getTexture("images/enemy4/enemy4.png");
        ship_hit = Texture.getTexture("images/enemy4/enemy4_hit.png");
        setImage(ship);
        ...
        engine1.setX(getX());
        engine1.setY(getY());
        engine1.setxOffsetMax(224);
        engine1.setxOffsetMin(215);
        engine1.setyOffsetMax(60);
        engine1.setyOffsetMin(55);
        engine1.setParticle(Texture.getTexture("images/bloom2.png"));
        engine1.setRMax(0.5f);
        engine1.setGMax(1f);
        engine1.setBMax(0.5f);
        engine1.setRMin(0.4f);
        engine1.setGMin(0.9f);
        engine1.setBMin(0.4f);
        ...
        engine1.setxSpeedMin(5);
        engine1.setxSpeedMax(8);
        engine1.setParticlesPerSecond(35);
        engine1.setScale(scale);
        engine1.setParticlesLayer(2);
        engine1.setDrawScale(scale * 0.5f);
    ...
    public void loop() {
        ...
        if (!Core.getInstance().isGamePaused()) {
            ...
            if (getX() < centerX + 512 && getX() > centerX - 1000) {
                ...
                //SPOSTIAMOCI!
                //if(!lockControls)y=  290+(float)Math.sin((double)cycle/96)*256;
                ...
                if (!kamikaze) {
                    ...
                    if (CommonVar.difficulty == CommonVar.DIFF_HARD && health / initHealth < ((float) kamikazeRandom / 100 + 0.25f)) {
                ...
                if (playerX + player.getWidth() <= getX()) {
                    if (canShot) {
                        SoundBox.play(2);
                        new EnemyPlasmaBall(getX() + 64 * scale, getY() + 64 * scale, scale, true, -9, -1, 4);
                        new EnemyPlasmaBall(getX() + 64 * scale, getY() + 64 * scale, scale, true, -9.5f, -0.5f, 4);
                        new EnemyPlasmaBall(getX() + 64 * scale, getY() + 64 * scale, scale, true, -10, 0, 4);
                        new EnemyPlasmaBall(getX() + 64 * scale, getY() + 64 * scale, scale, true, -9.5f, 0.5f, 4);
                        new EnemyPlasmaBall(getX() + 64 * scale, getY() + 64 * scale, scale, true, -9, 1, 4);
                        canShot = false;
...
//                    new Particle(Texture.getTexture("images/shockWave.png"), null, false, getX() + getWidth() / 2, getY() + getHeight() / 2, 0, 0, 1000, 10, 10, 100, 500, scale, 1, 1, 1, 1, 3, 0, 0, 0, 0, 0, 0.01f);
                     ...
                     */                }
                if (toDestroy) {
                    if(engine1!=null) engine1.destroy();
                    destroy();
                }
                explode = false;
            }
