...
public class Emitter extends Sprite implements ILoopable {
    ...
    private Texture particle = Texture.getTexture("images/defaultParticle.png");
    private Animo animazione = null;
    ...
    private boolean enableChecks = false, useanimazione = false, enabled = true, timed = false, additiveRender = false;
    ...
    public void setParticlesPerSecond(int particlesPerSecond) {
        this.particlesPerSecond = (int) (particlesPerSecond * mygame.CommonVar.gfxQuality);
    }
    ...
    public Emitter() {
        super(0f, 0f, Texture.getTexture("images/nullCursor.png"));
        this.setSmoothImage(CommonVar.smoothing);
    }

    public Emitter(float x, float y) {
        super(x, y, Texture.getTexture("images/nullCursor.png"));
        this.setSmoothImage(CommonVar.smoothing);
    }
        ...
        public void loop() {
            ...
            if (!Core.getInstance().isGamePaused()) {
                ...
                if (cycle != 0) {
                    ...
                    setY(getY() - (incY * speedMul) / 2);
                    setX(getX() - (incX * speedMul) / 2);
                }
    ...
    public void makeParticle() {
        ...
        new Particle(particle, animazione, useanimazione, getX() + (xOffset * scale), getY() + (yOffset * scale), xSpeed * scale, ySpeed * scale, life, xSizeInc * scale, ySizeInc * scale, fadeIn, fadeOut, scale, R, G, B, opacity, particlesLayer, initialRotation, rotRate, xAcceleration * scale, yAcceleration * scale, rotRateInc, drawScale, additiveRender);
    }
