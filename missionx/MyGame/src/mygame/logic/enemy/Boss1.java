 ...
 * @author Do$$e
 */
public class Boss1 extends SpriteColl implements ILoopable {
    ...
    boolean canShot = true, explode = false, toDestroy = false, lockControls = false, makeCollisions = true, timer2 = true, inBattle = false,genExp=false;
    LifeBar lb;
    ...
    public Boss1(float x, float y, int health) {
    ...
    public void init() {
        ...
        ship = Texture.getTexture("images/boss1/boss1.png");
        ship_hit = Texture.getTexture("images/boss1/boss1_hit.png");
        setImage(ship);
        ...
        engine1.setX(getX());
        engine1.setY(getY());
        engine1.setxOffsetMax(300);
        engine1.setxOffsetMin(290);
        engine1.setyOffsetMax(105);
        engine1.setyOffsetMin(100);
    ...
    public void loop() {
        ...
        if (!Core.getInstance().isGamePaused()) {
            ...
            if (getX() < centerX + 512 && getX() > centerX - 1000) {
                ...
                move((float) Math.sin((double) cycle / 60) * 2, (float) Math.sin((double) cycle / 60) * 2);
                if (getY() < 0) {
                    setY(0);
                }
                ...
                if (playerX + player.getWidth() <= getX()) {
                    if (canShot && playerY + player.getHeight() >= getY() + (32 * scale) && playerY <= getY() + getHeight() - (32 * scale) && timer2) {
                        SoundBox.play(15);
                        new EnemyGreenLaser(getX(), getY() + (125 * scale), scale * 0.5f, true, -40, 0, 5);
                        canShot = false;
